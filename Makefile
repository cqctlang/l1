...
clean: testclean
	...
	@$(MAKE) -s -C udis clean
	@$(MAKE) -s -C demo clean
	@$(RM) *~ .gdbhistory core core.* callgrind.out.* vgcore.* c.output l1.names main.o l1.o fns.*.c *.o $(TARG) *.so *.dylib mkelf mks l1.heap heap.S depend tl1 c.tab.c c.tab.h lex.yy.c
